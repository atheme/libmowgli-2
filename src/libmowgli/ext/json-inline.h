...
/* string */
static inline void mowgli_json_string_reset(mowgli_json_t *s)
{
	mowgli_string_reset(MOWGLI_JSON_STRING(s));
}

static inline void mowgli_json_string_append(mowgli_json_t *s, const char *src, size_t n)
{
	mowgli_string_append(MOWGLI_JSON_STRING(s), src, n);
}

static inline void mowgli_json_string_append_char(mowgli_json_t *s, const char c)
{
	mowgli_string_append_char(MOWGLI_JSON_STRING(s), c);
}

/* array */
static inline size_t mowgli_json_array_size(mowgli_json_t *arr)
{
	return MOWGLI_JSON_ARRAY(arr)->count;
}

static inline void mowgli_json_array_add(mowgli_json_t *arr, mowgli_json_t *data)
{
	mowgli_node_add(mowgli_json_incref(data), mowgli_node_create(), MOWGLI_JSON_ARRAY(arr));
}

static inline void mowgli_json_array_add_head(mowgli_json_t *arr, mowgli_json_t *data)
{
	mowgli_node_add_head(mowgli_json_incref(data), mowgli_node_create(), MOWGLI_JSON_ARRAY(arr));
}

static inline void mowgli_json_array_insert(mowgli_json_t *arr, mowgli_json_t *data, size_t pos)
{
	mowgli_node_insert(mowgli_json_incref(data), mowgli_node_create(), MOWGLI_JSON_ARRAY(arr), pos);
}

static inline void mowgli_json_array_delete(mowgli_json_t *arr, mowgli_json_t *data)
{
...
static inline bool mowgli_json_array_contains(mowgli_json_t *arr, mowgli_json_t *data)
{
	return mowgli_node_find(data, MOWGLI_JSON_ARRAY(arr)) != NULL;
}

static inline mowgli_json_t *mowgli_json_array_nth(mowgli_json_t *arr, size_t pos)
{
	return (mowgli_json_t*)mowgli_node_nth_data(MOWGLI_JSON_ARRAY(arr), pos);
}

/* object */
static inline size_t mowgli_json_object_size(mowgli_json_t *obj)
{
	return mowgli_patricia_size(MOWGLI_JSON_OBJECT(obj));
}

static inline void mowgli_json_object_add(mowgli_json_t *obj, const char *key, mowgli_json_t *data)
{
